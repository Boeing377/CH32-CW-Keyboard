...
uint8_t CAN_Init (CAN_TypeDef *CANx, CAN_InitTypeDef *CAN_InitStruct) {
    ...
    if ((CANx->STATR & CAN_STATR_INAK) != CAN_STATR_INAK) {
        ...
        CANx->BTIMR = (uint32_t)((uint32_t)CAN_InitStruct->CAN_Mode << 30) | ((uint32_t)CAN_InitStruct->CAN_SJW << 24) | ((uint32_t)CAN_InitStruct->CAN_BS1 << 16) | ((uint32_t)CAN_InitStruct->CAN_BS2 << 20) | ((uint32_t)CAN_InitStruct->CAN_Prescaler - 1);
...
void CAN_FilterInit (CAN_FilterInitTypeDef *CAN_FilterInitStruct) {
    ...
    if (((*(uint32_t *)0x40022030) & 0x0F000000) == 0) {
        ...
        for (i = 0; i < 64; i++) {
            *(__IO uint16_t *)(0x40006000 + 512 + 4 * i) =
                *(__IO uint16_t *)(0x40006000 + 768 + 4 * i);
        }
    }
...
uint8_t CAN_Transmit (CAN_TypeDef *CANx, CanTxMsg *TxMessage) {
    ...
    if (transmit_mailbox != CAN_TxStatus_NoMailBox) {
        ...
        CANx->sTxMailBox[transmit_mailbox].TXMDLR =
            (((uint32_t)TxMessage->Data[3] << 24) | ((uint32_t)TxMessage->Data[2] << 16) | ((uint32_t)TxMessage->Data[1] << 8) | ((uint32_t)TxMessage->Data[0]));
        CANx->sTxMailBox[transmit_mailbox].TXMDHR =
            (((uint32_t)TxMessage->Data[7] << 24) | ((uint32_t)TxMessage->Data[6] << 16) | ((uint32_t)TxMessage->Data[5] << 8) | ((uint32_t)TxMessage->Data[4]));
        CANx->sTxMailBox[transmit_mailbox].TXMIR |= TMIDxR_TXRQ;
    }
