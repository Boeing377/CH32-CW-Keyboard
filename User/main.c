...
/* the page → old data survives → we can validate it at boot.          */

static uint8_t DetectLegacyUpgrade (void)
{
    /* We read the old config as raw bytes — NOT through struct Config.
...
int main (void) {
    ...
    /* the chip was flashed from blank → refuse to run.            */
    /* ─────────────────────────────────────────────────────────── */
    if (!DetectLegacyUpgrade()) {
        show_piracy_warning();
        while (1) {
            /* Halt — firmware must be installed as an upgrade */
        }
    }
#endif
...
void ReadConfigEEPROM(){
    ...
    StorageBackend_ReadConfigRegion ((uint8_t *)&savedConfig,
                                     sizeof (struct Config));
    if (savedConfig.initial_startup != STARUP_FLAG)  // 确锟斤拷锟角凤拷锟斤拷锟斤拷锟斤拷锟斤拷锟斤拷舜锟轿?锟斤拷锟斤拷锟斤拷锟斤拷
    {
        config.initial_startup = STARUP_FLAG;
        config.mode = 0;
