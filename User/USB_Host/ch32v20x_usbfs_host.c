...
void USBFS_Host_Init(FunctionalState sta) {
    if (sta == ENABLE) {
        ...
        USBFSH->BASE_CTRL = USBFS_UC_HOST_MODE | USBFS_UC_INT_BUSY
                | USBFS_UC_DMA_EN;
    ...
    if ( USBFSH->INT_FG & USBFS_UIF_DETECT) // Check that there is a device connection or disconnection event on the port
    {
        ...
        if ( USBFSH->MIS_ST & USBFS_UMS_DEV_ATTACH) // Check that there is a device connection to the port
        {
            if ((dev_sta == ROOT_DEV_DISCONNECT)
                    || ((dev_sta != ROOT_DEV_FAILED)
                            && (USBFSH_CheckRootHubPortEnable() == 0x00))) {
...
uint8_t USBFSH_CheckRootHubPortSpeed(void) {
    return (
    USBFSH->MIS_ST & USBFS_UMS_DM_LEVEL ? USB_LOW_SPEED : USB_FULL_SPEED);
}
...
void USBFSH_SetSelfAddr(uint8_t addr) {
    USBFSH->DEV_ADDR = ( USBFSH->DEV_ADDR & USBFS_UDA_GP_BIT)
...
uint8_t USBFSH_Transact(uint8_t endp_pid, uint8_t endp_tog, uint16_t timeout) {
    ...
    do {
        ...
        for (i = DEF_WAIT_USB_TRANSFER_CNT;
                (i != 0) && (( USBFSH->INT_FG & USBFS_UIF_TRANSFER) == 0);
                i--) {
...
uint8_t USBFSH_CtrlTransfer(uint8_t ep0_size, uint8_t *pbuf, uint16_t *plen) {
    ...
    s = USBFSH_Transact(( USB_PID_SETUP << 4) | 0x00, 0x00,
    DEF_CTRL_TRANS_TIMEOVER_CNT);  // SETUP stage
    if (s != ERR_SUCCESS) {
        return s;
    }
    USBFSH->HOST_TX_CTRL = USBFSH->HOST_RX_CTRL = USBFS_UH_T_TOG
            | USBFS_UH_R_TOG; // Default DATA1
    ...
    s = USBFSH_Transact(
            ( USBFSH->HOST_TX_LEN) ?
                    ( USB_PID_IN << 4 | 0x00) : ( USB_PID_OUT << 4 | 0x00),
 ...
 */
uint8_t USBFSH_GetConfigDescr(uint8_t ep0_size, uint8_t *pbuf, uint16_t buf_len,
        uint16_t *pcfg_len) {
    uint8_t s;
...
uint8_t USBFSH_GetStrDescr(uint8_t ep0_size, uint8_t str_num, uint8_t *pbuf) {
    ...
    pUSBFS_SetupRequest->wValue = ((uint16_t) USB_DESCR_TYP_STRING << 8)
            | str_num;
 ...
 */
uint8_t USBFSH_GetEndpData(uint8_t endp_num, uint8_t *pendp_tog, uint8_t *pbuf,
        uint16_t *plen) {
    uint8_t s;
 ...
 */
uint8_t USBFSH_SendEndpData(uint8_t endp_num, uint8_t *pendp_tog, uint8_t *pbuf,
        uint16_t len) {
    uint8_t s;
